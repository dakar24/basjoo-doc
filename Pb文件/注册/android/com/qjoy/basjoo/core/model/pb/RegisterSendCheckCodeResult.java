// Code generated by Wire-Lite protocol buffer compiler, do not edit.
// Source file: ///home/admin/protobuf/src/fd5db16300abed243be25b1f20df81aa/proto/RegisterUser
package com.qjoy.basjoo.core.model.pb;

import com.squareup.wire.Message;
import com.squareup.wire.ProtoField;

import static com.squareup.wire.Message.Datatype.STRING;
import static com.squareup.wire.Message.Label.REQUIRED;

/**
 * 发送注册验证码结果
 */
public final class RegisterSendCheckCodeResult extends Message {

  public static final int TAG_RESULT_CODE = 1;
  public static final int TAG_RESULT_MESSAGE = 2;

  public static final String DEFAULT_RESULT_CODE = "";
  public static final String DEFAULT_RESULT_MESSAGE = "";

  @ProtoField(tag = 1, type = STRING, label = REQUIRED)
  public String result_code;

  @ProtoField(tag = 2, type = STRING, label = REQUIRED)
  public String result_message;

  public RegisterSendCheckCodeResult(RegisterSendCheckCodeResult message) {
    super(message);
    if (message == null) return;
    this.result_code = message.result_code;
    this.result_message = message.result_message;
  }

  public RegisterSendCheckCodeResult() {
  }

  public RegisterSendCheckCodeResult fillTagValue(int tag, Object value) {
    switch(tag) {
        case TAG_RESULT_CODE:
        this.result_code = (String)value;
        break;
        case TAG_RESULT_MESSAGE:
        this.result_message = (String)value;
        break;
        default: break;
        };
    return this;
  }

  @Override
  public boolean equals(Object other) {
    if (other == this) return true;
    if (!(other instanceof RegisterSendCheckCodeResult)) return false;
    RegisterSendCheckCodeResult o = (RegisterSendCheckCodeResult) other;
    return equals(result_code, o.result_code)
        && equals(result_message, o.result_message);
  }

  @Override
  public int hashCode() {
    int result = hashCode;
    if (result == 0) {
      result = result_code != null ? result_code.hashCode() : 0;
      result = result * 37 + (result_message != null ? result_message.hashCode() : 0);
      hashCode = result;
    }
    return result;
  }
}
